...
clear
clc
FolderExp='C:\Users\Troy\Desktop\From usb\Troys thing\Neural Network thing\front\single test';
FolderFig=FolderExp;
cd(FolderExp)
sampling=50;
freq_scan=[0.2,0.5];
T_scan=305;
name_scan{1}='T305_DCAC_05Hz';
...
%         close all
        ...
        %% choose proper NN
        FolderNNSP='C:\Users\Troy\Desktop\From usb\Troys thing\Neural Network thing\front\SPNN';
        FolderNNIP='C:\Users\Troy\Desktop\From usb\Troys thing\Neural Network thing\front\IPNN';
        FolderNNOther='C:\Users\Troy\Desktop\From usb\Troys thing\Neural Network thing\front\T325';
        ...
        cd(FolderNNOther) %% NN based on Other
        load input_Q.mat
        input_trainOther=theta(:,input_Q);
        load param.nn
        shiftt=load('shift.nn');
        factorr=load('factor.nn');
        thetam_expOther=apply_NN(param,shiftt,factorr,input_trainOther);
        output_expOther=thetam_expOther(:,end);
        output_expOther=output_expOther';
      
        
        cd(FolderFig)
        h7=figure(7);%% single NN plot
        plot(Time,output_expIP,'.k',Time,output_expSP,'.r',Time,output_expOther,'.g');grid on
        ylabel('predicted temperature (K)','fontsize',15)
        xlabel('Time (Sec)','Fontsize',15)
        legend('IP NN','SP NN','Other')
