...
% FolderExp='C:\Users\Troy\Dropbox\3 - Belgium Spider Silk\From usb\Troys Thing\Neural Network thing\Training files for students\Calibration done in May';
FolderExp='C:\Users\Troy\Dropbox\3 - Belgium Spider Silk\Programs\From Liwang\Neural Network thing\front';
cd(FolderExp)
% sampling=250;
sampling=50;
...
F_scan=15:-1:1;
theta0=[];
%Naming convention for getting calibration data.  The data is then loaded
%into columns in theta0
...
%Filling baseline spectra (which is 2nd column of theta)
...
%     poly_coeff(isp,:)=polyfit(log(Spectra_range),log(Spectra_av(isp,Spectra_range)),6);
end
...
%     poly_coeff_all(isp,:)=polyfit(log(Spectra_range),log(Spectra_all(isp,Spectra_range)),6);
end
theta_all=[T_pt1000_all,I_peak_all,I_integ_all,Ratio_all,FWHM_all,Peak_WL_all,Spectra_all(:,Spectra_range)];%,poly_coeff_all];
theta_all_norm=[T_pt1000_all,I_peak_all,I_integ_all,Ratio_all,FWHM_all,Peak_WL_all,Spectra_norm_all(:,Spectra_range)];%,poly_coeff_all];
save(['theta_all'],'theta_all','-ascii','-tabs');
save(['theta_all_norm'],'theta_all_norm','-ascii','-tabs');
%% Create curve fitted polynomial
% for isp=1:size(theta0,1)
%     fit_curve(isp,:)=polyval(poly_coeff(isp,:),log(Spectra_range));
% end
% 
% for isp=1:size(theta0,1)
%     fit_curve_all(isp,:)=polyval(poly_coeff_all(isp,:),log(Spectra_range));
% end
% 
% h99=figure(99);
% box on
% grid on
% hold on
% plot(WL800(Spectra_range),exp(fit_curve(1,:)))
...
%%
% input_Q=[1,2,4,round(linspace(5,606,80))];%%5,80-------------------------------------------
input_Q=[1,2,3];  %%5,80-------------------------------------------
...
param(12)=35;               % iteration times
param([13,14])=[cg1,size(input_cg,1)]; % test rows,from param 13 to 14
param([15,16])=[4,1];       % hidden layers,output units  %%5,80-------------------------------------------
param(17)=1;       % norm
...
name33=['T_NN'];
% savefigure(name33,'s',[6.6 5],'po','-.png')
...
ylabel('Output temperature (K)','Fontsize',10);xlabel('Target temperature (K)','Fontsize',10)
name44=['T_NN_AV'];
% savefigure(name44,'s',[6.6 5],'po','-.png')
