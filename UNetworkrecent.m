...
valtemps = valtemps.';
images = images-mean(mean(mean(mean(images))));
valimages = valimages-mean(mean(mean(mean(valimages))));
maxval = max(max(max(max(max(images)))),max(max(max(max(valimages)))));
images = images/maxval;
valimages = valimages/maxval;

 layer1 = [    
     imageInputLayer([c,c,3], 'Name', 'input')
     batchNormalizationLayer('Name','b_0')
